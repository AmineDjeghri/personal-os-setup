...
- Project: `/config/workspace/personal-os-setup/docs/home-server/music/youtube_ai_download/` — script `yt_dl.py` with a **PEP 723 `# /// script` header** (yt-dlp, mutagen; `requires-python >=3.14`; uv fetches a managed Python). Run via **`uv run --no-project yt_dl.py`** from that folder. Archive `.archive.txt`, staging `.staging` (outside `/media/music` so beets never imports half-finished folders), overrides `overrides.json`. README.md explains the pipeline.
