...
## A bare `require()` in `hyprland.lua` can silently kill every keybind

`hyprland.lua`'s top-level `require(...)` calls used to be unguarded. If a reload ever caught a required module (e.g. `variables.lua`) transiently missing or broken — plausible mid-sync, since chezmoi's own reload trigger races the file writes — that `require` threw and **aborted the rest of the file**, so nothing after it (including `binds.lua`) ever loaded, and `hyprctl configerrors` latched the error until a later reload happened to catch every file valid at once (not a timing issue you can fix with `sleep`). Worse, `binds.lua` read globals like `TERMINAL`/`APP_LAUNCHER` directly from `variables.lua` at load time with no nil-check, so even a *contained* failure in `variables.lua` alone crashed `binds.lua` too, silently dropping every keybind. Fixed two ways: wrapped each top-level require in a `pcall`-based `requireModule()` helper (so one bad module can't take down the rest), and — since `variables.lua` was the single most load-bearing module — deleted it and moved its contents directly into the top of `hyprland.lua`, which isn't itself fetched via `require` and so can't fail this way.
