...
- **Base a template on what the UI built from his live configuration** (v2.34 create-template/share flow) whenever he hands one over: that output is current-schema — `formatter.definitions` is a MAP keyed by formatter name, plus `tags`, `setToSaveInstallMenu`, `source: external` — and it already placeholders `rpdbApiKey`, `tmdbApiKey`, `tmdbAccessToken`. The repo export above is OLDER schema (singular `formatter.definition`, no `definitions` map), so use it as a cross-check only: rebuilding from it ships the pre-2.32 formatter shape.
...
2. **Convert with a throwaway snippet, never a maintained script** (his call, Sep 2026 — no tooling to keep in sync with a moving schema): move the config body under `config:`, add `metadata` (`name`, `description`, `author`, `category` required; `id` namespaced `author.my-template`, `source: external`, `version`, inline `changelog`, `sourceUrl`), drop the per-user `trusted` flag. Then print the written file's URL list, config-key count and any non-empty credential, and read that summary instead of the raw JSON.
...
4. Then de-personalize what is left: `addonDescription` (an export copied from someone else's instance carries their blurb and URL — replace with generic text), every addon `manifestUrl` not on the generic keep-list (Cinemeta), TMDB/indexer keys → `inputs.*`, `<template_placeholder>`, `__if`, `__switch` or `__remove`. Report the pruned catalog entries and the URLs that were placeholdered before you publish — he decides what to keep.
...
- **Sharing placeholders credentials, API keys and addon passwords ONLY.** Addon URLs, regex patterns, variant scripts and every other free-text field go in as written — read the JSON for anything personal before it is uploaded. A debrid key hides **percent-encoded inside a custom addon's URL** (statusio-style addons hold their whole config in the URL, `"ad_key":"<key>"` after decoding), so a `"apiKey"`-style grep finds nothing — decode every `manifestUrl` and judge it. When a key is in there, say **rotate it at the provider**: scrubbing the file is not enough. An account-bound URL with no key at all (Lumio, Streamfusion, per-user UUIDs, Trakt lists) is still personal: `<optional_template_placeholder>` + `enabled: false`.
- **Placeholdering an addon leaves its `catalogModifications` entries behind, and they must go too.** Their ids embed the source config's addon instance ids and, for Trakt, the list owner (`trakt_list:<username>:<list-id>`) — a stranger's own addon URL can never satisfy them, and publishing one leaks that username. Prune the entries whose `addonName` you placeholdered; the generic-id ones (`netflix-*`, `tvmio-*`, `4a0e3b0.*`) keep working for whoever adds their own URL, so offer them back to him rather than deciding unilaterally.
