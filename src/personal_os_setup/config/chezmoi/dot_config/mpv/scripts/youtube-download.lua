...
local opts = {
    ...
    -- Same as yt-dlp --cookies-from-browser. Auto-populated from mpv.conf's
    -- ytdl-raw-options if it sets cookies-from-browser=... (see below).
...
-- Also pick up cookies-from-browser=... from mpv.conf's ytdl-raw-options,
-- so the same browser cookie auth used for playback (YouTube bot-check
-- workaround) is reused for downloads.
if ytdl_raw_options ~= nil and ytdl_raw_options:find("cookies%-from%-browser=") ~= nil then
    local cookies_from_browser = ytdl_raw_options:match("cookies%-from%-browser=([^,]+)")
    if cookies_from_browser ~= nil then
...
local function download(download_type, config_file, overwrite_opts)
    ...
    elseif select_range_mode == 0 or (select_range_mode > 0 and (download_type == DOWNLOAD.AUDIO or download_type == DOWNLOAD.SUBTITLE)) then
        ...
        if not_empty(opts.cookies_from_browser) then
            table.insert(command, "--cookies-from-browser")
            table.insert(command, opts.cookies_from_browser)
        end
        table.insert(command, url)
